...
import { ShieldAlert, Lock, ShieldCheck, Loader2, Key } from 'lucide-react';
...
export const AdminLogin: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!email || !password) {
      setError('Please fill in admin credentials.');
      setIsLoading(false);
      return;
    }
    ...
    <div className="flex-grow flex items-center justify-center py-12 px-4 bg-slate-100 select-none">
      <div className="bg-white p-8 rounded-2xl border shadow-xl w-full max-w-md relative overflow-hidden">
        ...
        <div className="text-center mb-6">
          ...
          <span className="text-[10px] text-slate-400 block mt-0.5 font-bold uppercase tracking-widest text-red-600">
        ...
        {success && (
          <div className="bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs p-3.5 rounded-lg mb-5 flex items-start gap-2">
            ...
            <span>Authorized. Redirecting to Administration Dashboard...</span>
          </div>
        )}

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="text-xs font-bold text-slate-600 block mb-1.5 uppercase tracking-wider">Admin Username / Email</label>
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-9 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-lg text-xs font-medium focus:outline-none focus:ring-2 focus:ring-ksp-gold focus:border-transparent transition"
              />
            </div>
          </div>

          <div>
            <label className="text-xs font-bold text-slate-600 block mb-1.5 uppercase tracking-wider">Admin Security Key</label>
            <div className="relative">
              <span className="absolute left-3 top-3.5 text-slate-400 pointer-events-none">
                <Lock size={16} />
              </span>
        ...
        <div className="mt-6 text-center">
          <p className="text-[10px] text-slate-400 m-0">
            ⚠️ System logs IP, time, and session metadata. Unauthorized attempts are monitored by the Cybersecurity Unit.
          </p>
        </div>
      </div>
    </div>
