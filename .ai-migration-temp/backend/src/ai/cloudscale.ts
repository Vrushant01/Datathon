...
const getApp = () => {
  if (!appSingleton) {
    // No Express req available here — same fallback CloudScaleRepository.ts
    // uses when initialized outside a request context.
    appSingleton = catalyst.initialize(undefined as any);
  }
  return appSingleton;
};
...
async function scanTable(table: string): Promise<any[]> {
  ...
  entry.promise = (async () => {
    ...
    entry.promise = null;
    aiLogger.info(`[CloudScale] Scanned ${table}: ${allItems.length} records`);
    return allItems;
  })();
...
export const getTableData = async (name: string): Promise<any[]> => {
  return scanTable(resolveTable(name));
};
