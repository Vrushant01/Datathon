...
Never write anything else inside the \`\`\`recharts block except raw JSON.`;
  ...
  prompt += `2a. $lookup/$unwind are NOT supported in aggregate pipelines — CloudScale has no join operator. The casemasters table already includes denormalized DistrictID and DistrictName fields (derived from PoliceStationID), so district-level questions can $group directly on DistrictName without any join.\n`;
