...
exports.getCatalystAccessToken = void 0;
...
const getCatalystAccessToken = async () => {
    const now = Date.now();
    ...
    if (!clientId || !clientSecret || !refreshToken) {
        throw new Error('Missing ZOHO_CLIENT_ID / ZOHO_CLIENT_SECRET / ZOHO_REFRESH_TOKEN env vars for QuickML LLM auth.');
    }
    ...
    if (!resp.ok) {
        ...
        logger_1.aiLogger.error(`Zoho token refresh failed: ${resp.status} ${text}`);
        throw new Error(`Zoho token refresh failed: ${resp.status}`);
    }
    const json = await resp.json();
    if (!json.access_token) {
        logger_1.aiLogger.error(`Zoho token refresh response missing access_token: ${JSON.stringify(json)}`);
...
exports.getCatalystAccessToken = getCatalystAccessToken;
