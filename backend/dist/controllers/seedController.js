...
exports.validateCloudScaleSeeding = exports.seedCloudScaleData = void 0;
...
const seedCloudScaleData = async (req, res) => {
    try {
...
exports.seedCloudScaleData = seedCloudScaleData;
const sleep = (ms) => new Promise(resolve => setTimeout(resolve, ms));
async function fetchAll(table, itemsData, pkField, tableName) {
    const { NoSQLItem } = require('zcatalyst-sdk-node/lib/no-sql');
    let allRecords = [];
    for (let i = 0; i < itemsData.length; i += 25) {
        ...
        while (!success && retries > 0) {
            try {
                ...
                retries--;
                if (retries === 0)
                    console.error(`Batch fetch error in ${tableName}: ${e.message}`);
...
const validateCloudScaleSeeding = async (req, res) => {
    try {
        const app = zcatalyst_sdk_node_1.default.initialize(req);
        const nosql = app.nosql();
        const seedPath = path_1.default.join(__dirname, '../seedData.json');
        ...
        for (let i = 0; i < tables.length; i++) {
            ...
            const pkField = pks[i];
            const expectedData = expected[tableName];
            ...
            if (tableName === 'actsectionassociations') {
                ...
                const seen = new Set();
                uniqueExpectedData = expectedData.filter((r) => {
        ...
        const casemasterIds = new Set(allData['casemasters'].map((c) => Number(c.CaseMasterID)));
        let invalidFormats = 0;
        const firsByYear = {};
        allData['casemasters'].forEach((c) => {
            const cno = String(c.CrimeNo || '');
            if (!cno.match(/^[0-9]{4}\/202[0-9]$/))
                invalidFormats++;
            crimeNos.add(cno);
        ...
        for (const year in firsByYear) {
            const nums = firsByYear[year].sort((a, b) => a - b);
        ...
        allData['actsectionassociations'].forEach((r) => {
            const key = `${r.CaseMasterID}:${r.Act_Section}`;
            if (caseActSections.has(key))
                duplicateActSections++;
        ...
        for (const t of tables) {
            ...
            const expectedCount = Array.isArray(expected[t]) ? expected[t].length : expected[t];
        ...
        const unitIds = new Set(allData['units'].map((u) => Number(u.UnitID)));
        const districtIds = new Set(allData['districts'].map((d) => Number(d.DistrictID)));
        let casemasterOrphanUnits = 0;
        let unitOrphanDistricts = 0;
        let empOrphanDistricts = 0;
        let missingRequired = 0;
        allData['casemasters'].forEach((c) => {
            if (!unitIds.has(Number(c.PoliceStationID)))
                casemasterOrphanUnits++;
            if (!c.CrimeNo || !c.IncidentFromDate || !c.PoliceStationID) {
                missingRequired++;
            }
        });
        allData['units'].forEach((u) => {
            if (!districtIds.has(Number(u.DistrictID)))
                unitOrphanDistricts++;
        });
        allData['employees'].forEach((e) => {
            if (!districtIds.has(Number(e.DistrictID)))
                empOrphanDistricts++;
        });
...
exports.validateCloudScaleSeeding = validateCloudScaleSeeding;
