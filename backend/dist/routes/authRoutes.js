...
const generateToken = (payload) => {
    return jsonwebtoken_1.default.sign(payload, process.env.JWT_SECRET || 'fallback-secret-for-demo', { expiresIn: '8h' });
};
    ...
    try {
        ...
        if (loginType === 'admin') {
            if ((idOrEmail.toLowerCase() === 'admin@ksp.gov.in' || idOrEmail.toLowerCase() === 'admin') &&
                ...
                const token = (0, exports.generateToken)(adminUser);
            ...
            const token = (0, exports.generateToken)(analyticsUser);
            ...
            if (passcode === 'password' || passcode === 'ksp123') {
                ...
                const token = (0, exports.generateToken)(officerUser);
