...
const getDashboardData = async (db) => {
    ...
    const counts = stationCounts.map((s) => s.count);
    ...
    allUnits.forEach((u) => unitToDistrict.set(Number(u.UnitID), Number(u.DistrictID)));
    const districtNames = new Map();
    allDistricts.forEach((d) => districtNames.set(Number(d.DistrictID), d.DistrictName));
    const unitNames = new Map();
    allUnits.forEach((u) => unitNames.set(Number(u.UnitID), u.UnitName));
    // Run detection on the full in-memory dataset — no extra DB calls
    const anomalyReport = (0, timeSeriesAnalysis_1.detectTemporalAnomalies)(cases, unitToDistrict, districtNames, unitNames);
    ...
    anomalyReport.anomalies.forEach(a => {
        alerts.push({
            ...
            reason: a.reason,
            recommendedActions: (0, recommendationEngine_1.generateRecommendations)(a.crimeType, a.severity === 'CRITICAL' ? 'Critical' : 'High')
        });
    });
    alerts.sort((a, b) => b.riskScore - a.riskScore);
    // 8. District Intelligence Network
    const districtData = await (0, districtIntelligence_1.getDistrictIntelligence)(db);
