...
exports.getDistrictIntelligence = void 0;
const getDistrictIntelligence = async (db) => {
    ...
    allCases.forEach((c) => {
        ...
        if (Number(c.CaseStatusID) === 1)
            record.pendingCases += 1;
        if ([1, 2].includes(Number(c.GravityOffenceID)))
            record.violentCrime += 1;
        if (Number(c.CaseCategoryID) === 4)
            record.economicCrime += 1;
    });
    ...
    allUnits.forEach((u) => unitToDistrict.set(u.UnitID, u.DistrictID));
    const districtMap = new Map();
    allDistricts.forEach((d) => districtMap.set(d.DistrictID, d.DistrictName));
    ...
    districtRollup.forEach((stats, dId) => {
        ...
        if (stats.pendingCases > highestPending.val)
            highestPending = { name, val: stats.pendingCases };
        if (stats.violentCrime > highestViolent.val)
            highestViolent = { name, val: stats.violentCrime };
        ...
        if (growth > highestGrowth.val)
            highestGrowth = { name, val: growth };
        if (growth < mostImproved.val)
            mostImproved = { name, val: growth };
        finalData.push({
            districtId: dId,
            name,
...
exports.getDistrictIntelligence = getDistrictIntelligence;
