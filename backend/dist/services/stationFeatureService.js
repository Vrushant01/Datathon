...
exports.calculateFeatures = void 0;
const RepositoryFactory_1 = require("../repositories/RepositoryFactory");
const calculateFeatures = async (req, stationId) => {
    ...
    const uniqueAccusedIds = new Set();
    accusedRecords.forEach(a => {
        if (a.PersonID)
            uniqueAccusedIds.add(a.PersonID);
        else if (a.AccusedName)
            uniqueAccusedIds.add(a.AccusedName.toLowerCase());
    });
    ...
    if (uniqueAccusedIds.size > 0) {
        ...
        if (validPersonIds.length > 0) {
            ...
            const casesWithRepeatOffenders = new Set();
...
exports.calculateFeatures = calculateFeatures;
