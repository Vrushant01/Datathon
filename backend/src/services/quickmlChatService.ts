...
export interface ToolCallArgs {
  ...
  district?: number;
  station?: number;
  crime_category?: number;
}

export class QuickMLChatService {
  
  /**
   * Executes the 'get_case_statistics' tool locally against CloudScale.
   */
  static async executeTool(req: any, args: ToolCallArgs): Promise<any> {
    aiLogger.info(`Executing tool get_case_statistics with args:`, args);
    const repo = RepositoryFactory.getRepository(req); // CloudScale repo requires req for headers
    ...
    try {
      ...
      aiLogger.info(`Tool execution successful. Result count/metric:`, result.metric);
      return result;
    } catch (error: any) {
      aiLogger.error(`CloudScale tool execution failed: ${error.message}`);
   ...
   */
  static async processMessage(req: any, question: string, history: any[]): Promise<string> {
    ...
    aiLogger.info(`Sending question to QuickML GLM-4.7-Flash: ${question}`);
    ...
    try {
      ...
      response = await axios.post(endpoint as string, payload, {
        headers: {
          'CATALYST-ORG': orgId,
      ...
      aiLogger.info(`QuickML response received with status ${response.status}`);
    } catch (e: any) {
      if (e.response) {
        aiLogger.error(`QuickML API Error: ${e.response.status} - ${JSON.stringify(e.response.data)}`);
      ...
      aiLogger.error(`QuickML request failed: ${e.message}`);
    ...
    if (toolCalls && toolCalls.length > 0) {
      ...
      if (toolCall.name === "get_case_statistics") {
         aiLogger.info(`QuickML requested tool call: get_case_statistics with args`, toolCall.parameters || toolCall.arguments);
       ...
       aiLogger.info(`No tool called. GLM answered directly.`);
    }
