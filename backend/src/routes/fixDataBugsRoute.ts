...
router.post('/', requireAuth, requireRole('Admin'), async (req: any, res: any) => {
    try {
        ...
        const isWithinDavanagere = (lat: number, lon: number) => {
            return lat >= minLat && lat <= maxLat && lon >= minLon && lon <= maxLon;
        };

        const unitsTable = nosql.table('units');
        let ids: number[] = [];
        for (let i = 2000; i <= 2930; i++) ids.push(i);
        ...
        let unitsToUpdate: any[] = [];
        let casesToUpdate: any[] = [];
        let skippedUnits = 0;
        let skippedCases = 0;
        let samples: any[] = [];
        ...
        if (commit) {
            ...
            for (const update of unitsToUpdate) {
                ...
                try {
                    await unitsTable.updateItems(updateItem as any);
            ...
            for (let i = 0; i < casesToUpdate.length; i += 20) {
                ...
                try {
                    await Promise.all(items.map(item => casesTable.updateItems(item as any)));
                } catch(e) {}
            }
