...
router.get('/dashboard', requireAuth, async (req, res) => {
  ...
  try {
    ...
    const filteredCases = cases.filter((c: any) => {
      if (targetDistrictId !== 'ALL') {
        const station = units.find((s: any) => s.UnitID === c.PoliceStationID);
    ...
    const solvedCases = filteredCases.filter((c: any) => c.CaseStatusID === 2 || c.CaseStatusID === 3 || c.CaseStatusID === 4).length;

    // 1. Crime by District / Station (Graph 1)
    let chart1Data: any[] = [];
    if (targetDistrictId === 'ALL') {
      chart1Data = districts.map((d: any) => {
        const districtStations = units.filter((s: any) => s.DistrictID === d.DistrictID);
        const caseCount = filteredCases.filter((c: any) => districtStations.some((s: any) => s.UnitID === c.PoliceStationID)).length;
        return { name: String(d.DistrictName || '').replace(' City', '').replace(' Rural', ''), Cases: caseCount };
      }).filter((item: any) => item.Cases > 0).sort((a: any, b: any) => b.Cases - a.Cases).slice(0, 10);
    } else if (targetStationId === 'ALL') {
      const districtStations = units.filter((s: any) => s.DistrictID === targetDistrictId);
      chart1Data = districtStations.map((s: any) => {
        const caseCount = filteredCases.filter((c: any) => c.PoliceStationID === s.UnitID).length;
        return { name: String(s.UnitName || '').replace(' PS', ''), Cases: caseCount };
      }).filter((item: any) => item.Cases > 0).sort((a: any, b: any) => b.Cases - a.Cases).slice(0, 10);
    } else {
      const s = units.find((s: any) => s.UnitID === targetStationId);
      chart1Data = s ? [{ name: String(s.UnitName || '').replace(' PS', ''), Cases: totalCases }] : [];
    }

    // 2. Crime Categories
    const categoryData = CRIME_HEADS.map((ch: any) => {
      const caseCount = filteredCases.filter((c: any) => c.CrimeMajorHeadID === ch.CrimeHeadID).length;
      return { name: String(ch.CrimeGroupName || '').split(' ').slice(-2).join(' '), Cases: caseCount };
    }).filter((c: any) => c.Cases > 0).sort((a: any,b: any) => b.Cases - a.Cases).slice(0, 8);

    // 3. Victim Age Demographics
    const victimAgeData = [
      { name: 'Under 18', Count: victims.filter((v: any) => v.AgeYear < 18 && filteredCases.some((c: any) => c.CaseMasterID === v.CaseMasterID)).length },
      { name: '18 - 30', Count: victims.filter((v: any) => v.AgeYear >= 18 && v.AgeYear <= 30 && filteredCases.some((c: any) => c.CaseMasterID === v.CaseMasterID)).length },
      { name: '31 - 50', Count: victims.filter((v: any) => v.AgeYear > 30 && v.AgeYear <= 50 && filteredCases.some((c: any) => c.CaseMasterID === v.CaseMasterID)).length },
      { name: 'Over 50', Count: victims.filter((v: any) => v.AgeYear > 50 && filteredCases.some((c: any) => c.CaseMasterID === v.CaseMasterID)).length }
    ].filter((v: any) => v.Count > 0);

    // 4. Accused Age Demographics
    const accusedAgeData = [
      { name: 'Under 18', Count: accused.filter((a: any) => a.AgeYear < 18 && filteredCases.some((c: any) => c.CaseMasterID === a.CaseMasterID)).length },
      { name: '18 - 30', Count: accused.filter((a: any) => a.AgeYear >= 18 && a.AgeYear <= 30 && filteredCases.some((c: any) => c.CaseMasterID === a.CaseMasterID)).length },
      { name: '31 - 50', Count: accused.filter((a: any) => a.AgeYear > 30 && a.AgeYear <= 50 && filteredCases.some((c: any) => c.CaseMasterID === a.CaseMasterID)).length },
      { name: 'Over 50', Count: accused.filter((a: any) => a.AgeYear > 50 && filteredCases.some((c: any) => c.CaseMasterID === a.CaseMasterID)).length }
    ].filter((a: any) => a.Count > 0);

    // 5. Officer Case Load
    const officerData = officers.map((o: any) => {
      const assignedCount = filteredCases.filter((c: any) => c.PolicePersonID === o.EmployeeID).length;
      const solvedCount = filteredCases.filter((c: any) => c.PolicePersonID === o.EmployeeID && (c.CaseStatusID === 2 || c.CaseStatusID === 3)).length;
      return { 
    ...
    }).filter((o: any) => o.Assigned > 0).sort((a: any,b: any) => b.Assigned - a.Assigned).slice(0, 10);

    // 6. Historical Registration Trend
    const monthlyCounts = new Map<string, { Cyber: number, Property: number, Body: number, total: number }>();
    filteredCases.forEach((c: any) => {
       ...
       counts.total++;
       if (c.CrimeMajorHeadID === 500) counts.Cyber++;
       else if (c.CrimeMajorHeadID === 200) counts.Property++;
       else if (c.CrimeMajorHeadID === 100) counts.Body++;
    });
