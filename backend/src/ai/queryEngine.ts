...
export const assertSafeQuery = (query: any) => {
...
const getField = (doc: any, path: string) =>
  path.split('.').reduce((v, k) => (v == null ? undefined : v[k]), doc);

export const matchesFilter = (doc: any, filter: Record<string, any> = {}): boolean => {
  for (const [key, cond] of Object.entries(filter)) {
    const value = getField(doc, key);
    if (cond !== null && typeof cond === 'object' && !Array.isArray(cond)) {
      for (const [op, opVal] of Object.entries(cond) as [string, any][]) {
        switch (op) {
...
export const findDocs = (data: any[], filter: any, limit = 10, sort?: [string, 1 | -1]) => {
  assertSafeQuery(filter);
  let results = data.filter(d => matchesFilter(d, filter));
  if (sort) {
...
export const countDocs = (data: any[], filter: any) => {
  assertSafeQuery(filter);
...
export const runAggregate = (data: any[], pipeline: any[]) => {
  ...
  for (const stage of pipeline) {
    if (stage.$match) {
      ...
      rows = Array.from(groups.entries()).map(([key, groupRows]) => {
        ...
        for (const [outField, accExpr] of Object.entries(accs) as [string, any][]) {
          const [accOp, srcField] = Object.entries(accExpr)[0] as [string, any];
          ...
          switch (accOp) {
            case '$sum': out[outField] = isCountShorthand ? groupRows.length : nums.reduce((a, b) => a + b, 0); break;
            case '$avg': out[outField] = nums.reduce((a, b) => a + b, 0) / (nums.length || 1); break;
            case '$min': out[outField] = Math.min(...nums); break;
            case '$max': out[outField] = Math.max(...nums); break;
            case '$count': out[outField] = groupRows.length; break;
      ...
      const [[field, dir]] = Object.entries(stage.$sort) as [string, 1 | -1][];
