...
async function fetchItemsByPKs(app: any, tableName: string, pkField: string, pkValues: number[]): Promise<any[]> {
  ...
  return (raw.get || []).map((d: any) => d.item ?? d);
}

// Query a GSI with EQUALS on pk, fully paginated
async function queryGSIPaginated(app: any, tableName: string, indexId: string, pkField: string, pkValue: number): Promise<any[]> {
  const nosql = app.nosql();
  const table = nosql.table(tableName);
  let allItems: any[] = [];
  ...
  do {
    ...
    if (startKey) query.start_key = startKey;
    const resp = await table.queryIndex(indexId, query);
    const raw = resp;
    const items = raw.get || [];
    allItems = allItems.concat(items.map((d: any) => {
      const item = d.item;
...
async function sampleByPKs(app: any, tableName: string, pkField: string, sampleIds: number[]): Promise<{ found: number; missing: number[] }> {
  const nosql = app.nosql();
  const table = nosql.table(tableName);
...
export const validateMigration = async (req: Request, res: Response) => {
  const t0 = Date.now();
  try {
    ...
    const [mgD, mgU, mgE, mgC, mgA, mgV] = await Promise.all([
      District.countDocuments(), Unit.countDocuments(), Employee.countDocuments(),
      CaseMaster.countDocuments(), Accused.countDocuments(), Victim.countDocuments(),
    ]);
    ...
    const allDistricts = await District.find().select('DistrictID').lean();
    const distIds = allDistricts.map(d => Number((d as any).DistrictID));
    const distCheck = await sampleByPKs(app, 'districts', 'DistrictID', distIds);

    const allUnits = await Unit.find().select('UnitID').lean();
    const unitIds = allUnits.map(u => Number((u as any).UnitID));
    // Sample 50
    ...
    const empIds = allEmps.map(e => Number((e as any).EmployeeID));
    ...
    const allCases = await CaseMaster.find().select('CaseMasterID').lean();
    const caseIds = allCases.map(c => Number((c as any).CaseMasterID));
    ...
    const allAccused = await Accused.find().select('AccusedMasterID').lean();
    const accIds = allAccused.map(a => Number((a as any).AccusedMasterID));
    ...
    const allVictims = await Victim.find().select('VictimMasterID').lean();
    const vicIds = allVictims.map(v => Number((v as any).VictimMasterID));
    ...
    const mgDistSample: any[] = await District.find().lean();
    const nosqlD = app.nosql();
    const districtSpotCheck: any[] = [];
    for (const mg of mgDistSample) {
      ...
      try {
        ...
        if (csItem) {
           const json = typeof (csItem as any).toJSON === 'function' ? (csItem as any).toJSON() : csItem;
    ...
    // ── 4. GSI query tests ────────────────────────────────────────────────────
    const gsiResults: Record<string, any> = {};
    const gsiTests = [
    ...
    let personCaseMap: Record<string, string[]> = {};
    for (let caseId = 1; caseId <= 20; caseId++) {
      try {
        ...
        rows.forEach((row: any) => {
          ...
          if (!pid) return;
          if (!personCaseMap[pid]) personCaseMap[pid] = [];
    ...
    const countReport = Object.entries(mgCounts).map(([tbl, mg]) => ({
      table: tbl,
      expected: (expected as any)[tbl],
      mongodb: mg,
      mongoMatch: mg === (expected as any)[tbl] ? 'PASS' : 'FAIL',
